...
class LossFunction():
    r"""
        Loss Function class, contains 5 methods, 4 of which are the actual loss function and 
        one which contains a list of all of them.
    """
...
class Learn():
        ...
        for epoch in tqdm(range(self.nb_epoch)):
            ...
            if epoch_no_improvement == 0:
                ...
                if not os.path.exists(
                        os.path.join(Folders.results_path, experiment_name)):
                    os.makedirs(
                        os.path.join(Folders.results_path, experiment_name))
                file_save = os.path.join(
                    Folders.results_path, experiment_name,
                    "{0}_{1}_{2}_{3}.csv".format(self.model_type.name,
                                                 str(self.nodes_size.name),
                                                 str(self.distanceType.name),
                                                 str(tune.get_trial_id())))
    ...
    def __val(self, dfResults: pd.DataFrame, epoch: int):
        ...
        for criterion in LossFunction.Criterions():
            ...
            results = {
                "Model": str(self.model_type.name),
    ...
    def __SARIMATrainAndTest(self, experiment_name: str) -> None:
        ...
        results_folder = os.path.join(Folders.results_ray_path,
                                      experiment_name,
                                      Constants.checkpoin_SARIMA_folder)

        if not os.path.exists(results_folder):
            os.makedirs(results_folder)
        dfResults = pd.DataFrame(columns=["Node_Id", "Criterion", "Loss"])
        for _, (X_train, X_test, Y_train, Y_test,
                node_id) in enumerate(self.train_dataset):
            ...
            for criterion in LossFunction.Criterions():
                loss = criterion(
                    torch.FloatTensor(Y_test).to(self.train_dataset.device),
                    torch.FloatTensor(Y_pred).to(self.train_dataset.device))
                loss = loss.item()
                result = {
                    "Node_Id": node_id,
                ...
                if criterion == LossFunction.MAE:
                    print(
                        "Best trial test set loss {0} : {1} for node id : {2}".
                        format(criterion.__name__, loss, node_id))
                    name_model = os.path.join(
                        results_folder, "model_node_{0}_{1}_{2}.pickle".format(
                            node_id, criterion.__name__, loss))
                    with open(name_model, 'wb') as handle:
                        pickle.dump(SARIMAX_model,
                                    handle,
    ...
    def __ARIMATrainAndTest(self, experiment_name: str) -> None:
        ...
        results_folder = os.path.join(Folders.results_ray_path,
                                      experiment_name,
                                      Constants.checkpoin_ARIMA_folder)
    ...
    def __LRTrainAndTest(self, experiment_name: str) -> None:
        ...
        results_folder = os.path.join(Folders.results_ray_path,
                                      experiment_name,
                                      Constants.checkpoint_LR_folder)
    ...
    def __set_for_train(self) -> None:
        ...
        if self.model_type == ModelType.STCONV:
            ...
            self.model = LSTMModel(node_features=self.num_features,
                                   hidden_channels=self.hidden_channels,
                                   K=self.K)
            ...
            self.train_dataset, self.validation_dataset, self.test_dataset = LSTMDataset.get_dataset_LSTM(
                ...
                datareader=self.datareader,
                device=self.device)
            self.model = DCRNNModel(node_features=self.num_features,
                                    hidden_channels=self.hidden_channels,
                                    K=self.K)
    ...
    def set_data(datareader: DataReader) -> None:
        r"""
            Function to call to make all prerequireing data processing such that everything 
            ...
            for sigma in Graph.sigma_array:
                for distanceType in DistanceType:
                    LinearRegressionDataset.set_graph_with_LR(
                        datareader, DatasetSize.Experimental, distanceType,
                        epsilon, sigma)
                    LinearRegressionDataset.set_graph_with_LR(
                        datareader, DatasetSize.Tiny, distanceType, epsilon,
                        sigma)
    ...
    def __start(config, param, checkpoint_dir=None) -> None:
        r"""
            ...
            Class Function
            Args:
                param : dict, contains learning_rate, num_features, EarlyStoppingPatience, 
                nb_epoch, datareader, nodes_size, train_ratio, val_ratio, test_ratio, criterion, 
                model_type
    ...
    def startNonGNN(datareader: DataReader, experiment_name: str,
    ...
    def HyperParameterTuning(datasetsize: DatasetSize, model: ModelType,
                             distanceType: DistanceType,
                             datareader: DataReader,
                             experiment_name: str) -> None:
        r"""
            Function for hyper parameter tuning, it receives a datasetsize, model type, data reader 
            and a loss function as a criterion, returns nothing.
        ...
        scheduler = ASHAScheduler(max_t=Constants.nb_epoch,
        ...
        config = {
            ...
            tune.choice([1, 3, 5, 7]),
            "hidden_channels":
            tune.choice([4, 8, 16, 32]),
            "epsilon":
            tune.choice([0.1]),
            "optimizer_type":
            tune.choice([
                OptimizerType.Adam, OptimizerType.AdamW, OptimizerType.Adamax,
            ...
            tune.choice([model]),
            "nodes_size":
            tune.choice([datasetsize])
        }

        if model == ModelType.DCRNN:
            config["K"] = tune.choice([1])

        if (datasetsize != DatasetSize.ExperimentalManual
                and datasetsize != DatasetSize.ExperimentalLR
                and datasetsize != DatasetSize.TinyManual
                and datasetsize != DatasetSize.TinyLR):
            config["epsilon"] = tune.choice([0.1, 0.3, 0.5, 0.7])
            config["sigma"] = tune.choice([1, 3, 5, 10])

        directory_experiment_ray = os.path.join(Folders.results_ray_path,
                                                experiment_name)

        Learn.__start.__name__ = f"{model.name}_{datasetsize.name}_{distanceType.name}"

        result = tune.run(tune.with_parameters(Learn.__start, param=param),
    ...
    def Run():
        ...
        Learn.startNonGNN(datareader=datareader,
                          experiment_name=experiment_name,
                          model_type=ModelType.LinearRegression)
        Learn.startNonGNN(datareader=datareader,
                          experiment_name=experiment_name,
